...
<br>
<br>
<center><font color="white" size="5">Withdraw Amount From Customer Account</font></center>
Customer ID<br />
...
Customer Name<br />
 ...
 <br />
Mail<br />
 <input name="emailid" type="text" value="<?php  echo $row['mail']; ?>" required />
 <br />
Account Number<br />
...
<br>
Balance<br />
 <input name="balance" id="balance" type="text" class="ed" required value="<?php  echo $row['balance']; ?>" />
<br>
 Withdraw Amount<br />
 <input type="text" name="amt" id="amt" required value="" class="ed"><br />
 <input type="button" name="add" onclick="sum()" value="Calculate" id="button" />
 <br>
  Available Amount<br />
  <input type="text" name="ttamt" id="ttamt" required value="" class="ed"><br />
  <input type="submit" name="submit" value="Withdraw" id="button1" />
  <br>
...
if(parseInt(balance) <= parseInt(amt))
	{
	 alert("Customer doesnt have enough balance");
	}
	else{
...
<?php
 ...
 if(isset($_POST['submit']))
	...
	$query = "INSERT INTO `trans` VALUES (null, '".$_POST['id']."', '".$_POST['name']."','".$_POST['emailid']."', '".$_POST['acno']."','".$_POST['amt']."','".$_POST['ttamt']."' , curdate(),'Debited')";
	$query1 = "update user set balance='".$_POST['ttamt']."' where id='".$_POST['id']."'";
          
	if(mysql_query($query) && mysql_query($query1)){
		echo ' SUCCESSFULLY';
	  echo '<script> alert("DEBITED SUCCESSFULLY");</script>';
	}
	else{
		echo 'NOT REGISTERD';
	}
	$amt=$_POST['amt'];
			...
			$email->set_subject("Amount Debited");
			//$email->set_message(html_entity_decode($html.'<br /><br />'));
			$email->set_message("Hi $name! Your Account was Debited with the amount of &#8377; $amt today, and your available balance now is &#8377; $ttamt. Thank You. Have a Great Day");
